...
import sys
import numpy
# Compatibility shim for models saved with NumPy 2.x being loaded with NumPy 1.x
...
from common.models import CustomCNN, RobustDoomCNN
...
def game_instance(scenario):
    ...
    config_path = os.path.join(script_dir, "scenarios", f"{scenario}.cfg")
...
if __name__ == "__main__":
    ...
    agent_args = {
        "n_steps": 4096,
        "learning_rate": 1e-4,
        "clip_range": 0.2, # Override to avoid pickle compatibility issues
        "ent_coef": 0.0,   # Override to avoid pickle compatibility issues
        "batch_size": 4096, # 64
        "policy_kwargs": {"features_extractor_class": RobustDoomCNN},
        "device": device,
    }
    ...
    if scenario in ["deathmatch", "deathmatch_simple"]:
        env = vec_env_with_bots_curriculum(n_envs, **env_args)
        eval_env = vec_env_with_bots_curriculum(n_envs, **eval_env_args)
    else:
        # Standard singleplayer scenarios do not support custom params like n_bots or shaping
        basic_env_args = {
            "scenario": scenario,
            "frame_skip": env_args["frame_skip"],
            "frame_processor": env_args["frame_processor"],
        }
        env = envs.create_vec_env(n_envs, **basic_env_args)
        eval_env = envs.create_eval_vec_env(**basic_env_args)
    ...
    agent = envs.solve_env(
        ...
        agent_args,
        resume=False,
        load_path="./ViZDoomPPO/logs/models/deathmatch_simple/best_model.zip"
    )
