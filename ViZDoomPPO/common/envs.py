...
class TensorboardGifCallback(BaseCallback):
    def __init__(self, log_freq: int = 4000, verbose=0):
    ...
    def _on_step(self):
        ...
        if self.collecting:
            ...
            if len(self.frames) >= 20:
                video_array = np.expand_dims(np.stack(self.frames), axis=0)
                if video_array.dtype != np.uint8:
                    if video_array.max() <= 1.0:
                        video_array = (video_array * 255).astype(np.uint8)
                    else:
                        video_array = video_array.astype(np.uint8)
                self.logger.record("trajectory/video", Video(video_array, fps=15))
                self.frames = []
                self.collecting = False
        return True

class FullEpisodeVideoCallback(BaseCallback):
    def __init__(self, log_freq: int = 10000, save_dir: str = "runs/videos", verbose=0):
        super().__init__(verbose)
        self.frames = []
        self.log_freq = log_freq
        self.wants_to_collect = False
        self.collecting = False
        ...
        if self.collecting:
            if done:
                ...
                if "terminal_observation" in info:
                    self.frames.append(info["terminal_observation"])
                
                if len(self.frames) > 0:
                    video_array = np.stack(self.frames)  # (T, C, H, W)
                    if video_array.dtype != np.uint8:
                        if video_array.max() <= 1.0:
                            video_array = (video_array * 255).astype(np.uint8)
                        else:
                    ...
                    save_path = os.path.join(self.save_dir, f"episode_step_{self.num_timesteps}.mp4")
                    video_array_hwc = np.transpose(video_array, (0, 2, 3, 1))
                    try:
                        imageio.mimsave(save_path, video_array_hwc, fps=25)
                ...
                self.frames.append(self.locals["new_obs"][0].copy())
                
        if self.wants_to_collect and done:
            self.collecting = True
            self.wants_to_collect = False
            self.frames = [self.locals["new_obs"][0].copy()]
...
def solve_env(env: vec_env.VecTransposeImage, eval_env: vec_env.VecTransposeImage, scenario: str, agent_args: t.Dict, resume: bool = False, load_path: str = None):
    ...
    if resume:
        ...
        if load_path != "":
            agent = ppo.PPO.load(load_path, env=env, tensorboard_log='runs/', **agent_args)
        ...
        # Create a new agent
        agent = ppo.PPO(policies.ActorCriticCnnPolicy, env, tensorboard_log='runs/', seed=0, **agent_args)
        init_model(agent)
    ...
    gif_callback = TensorboardGifCallback(log_freq=5000)
    full_epi_callback = FullEpisodeVideoCallback(log_freq=100000)
    ...
    try:
        agent.learn(
            ...
            callback=[eval_callback, progress_callback, gif_callback, full_epi_callback],
